...
for (printer, texture), tests in experimental_data.items():
    ...
    current_method_error = method_errors.get((printer,texture), 0.0)
    current_method_std=std_error.get((printer,texture), 0.0)
...
if not df_raw.empty:
    ...
    for col in numeric_cols:
        df_grouped[col] = df_grouped[col].map(lambda x: f"{x:.8f}")
